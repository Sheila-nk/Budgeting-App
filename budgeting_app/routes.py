...
@login_required
def user_dashboard():
    # Create a user dashboard where a user can input their income and spending
    depositform = DepositForm()
    spendingform = SpendingForm()

    if (
        depositform.deposit_category.data
        and depositform.deposit_amount.data
        and depositform.validate_on_submit()
    ):
    ...
    if (
        spendingform.withdraw_category.data
        and spendingform.withdraw_amount.data
        and spendingform.validate_on_submit()
    ):
    ...
    result = Budget.query.filter_by(user_id=current_user.id).with_entities(
        Budget.deposit_category,
        Budget.deposit_amount,
        Budget.withdraw_category,
        Budget.withdraw_amount,
    ...
    return render_template(
        ...
        depositform=depositform,
        spendingform=spendingform,
        d_headings=d_headings,
        w_headings=w_headings,
        deposit_ledger=deposit_ledger,
        withdraw_ledger=withdraw_ledger,
    )
...
def user_transactions():
    # Display all the user's transactions
    headings = (
        "Date Posted",
        "Income",
        "Description",
    ...
    result = Budget.query.filter_by(user_id=current_user.id).with_entities(
        Budget.date_posted,
        Budget.deposit_category,
        Budget.deposit_description,
        Budget.deposit_amount,
        Budget.withdraw_category,
        Budget.withdraw_description,
        Budget.withdraw_amount,
    )
